...
if [ -f "remnawave-vars.sh" ]; then
    echo "File remnawave-vars.sh already exists."
    echo "Do you want to recreate it? This will generate new credentials! (y/N)"
    read -r response
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
        echo "Loading existing variables..."
        source remnawave-vars.sh
        echo
        echo "✓ Existing environment variables loaded!"
        exit 0
    fi
    echo "Backing up existing file to remnawave-vars.sh.backup"
    cp remnawave-vars.sh remnawave-vars.sh.backup
fi
...
Cookie Auth: ${cookies_random1}=${cookies_random2}
========================================
...
echo
echo "Credentials saved to: remnawave-credentials.txt"
echo "To view credentials later, use: cat remnawave-credentials.txt"
echo
