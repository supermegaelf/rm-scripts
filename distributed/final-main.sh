...
echo -e "\n=== SUBSCRIPTION PAGE CHECK ==="

SUB_CHECK=$(curl -s -o /dev/null -w "%{http_code}" "https://$SUB_DOMAIN")

if [ "$SUB_CHECK" == "200" ]; then
    print_success "Subscription page is accessible"
else
    print_error "Subscription page issue, code: $SUB_CHECK"
fi
