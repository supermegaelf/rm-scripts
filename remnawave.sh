...
EOF
    ...
    warning "Please edit remnawave-vars.sh and fill in your domain and Cloudflare credentials:"
    echo "nano remnawave-vars.sh"
    echo ""
    echo "Press Enter after editing the file to continue..."
    read
    
    if [[ ! -f "remnawave-vars.sh" ]]; then
        error "remnawave-vars.sh not found!"
    fi
    ...
    if [[ -z "$PANEL_DOMAIN" || -z "$SUB_DOMAIN" || -z "$SELFSTEAL_DOMAIN" || -z "$CLOUDFLARE_API_KEY" || -z "$CLOUDFLARE_EMAIL" ]]; then
        error "Please fill in all required variables in remnawave-vars.sh"
    fi
...
setup_ssl() {
    ...
    if ! echo "$api_response" | jq -e '.success' > /dev/null 2>&1; then
        error "Failed to verify Cloudflare API credentials"
    fi
...
display_results() {
    ...
    echo "-------------------------------------------------"
    echo "To log into the panel, use the following data:"
